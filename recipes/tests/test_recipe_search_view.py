...
class RecipeSearchViewsTest(RecipeTestBase):
    def test_recipe_search_view_uses_correct_view_function(self):
    ...
    def test_recipe_search_can_find_recipe_by_description(self):
        ...
        recipe1 = self.make_recipe(
            slug='one', description=description1, author_data={'username': 'one'}
        )

        recipe2 = self.make_recipe(
            slug='two', description=description2, author_data={'username': 'two'}
        )

        search_url = reverse('recipes:search')
        response1 = self.client.get(f'{search_url}?q={description1}')
        response2 = self.client.get(f'{search_url}?q={description2}')
        response_both = self.client.get(f'{search_url}?q=thiss')
